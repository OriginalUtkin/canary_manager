...
from canary_hitman.clients import GitlabClient, JSON
from canary_hitman.models import CommitterInfo, Commit, Release
...
class Hitman:
    def __init__(
            self, k8s_namespace: str, canary_names: str, ttl: str, repository_name: str, commit_to_release_sha: str
    ) -> None:
        self.k8s_namespace: str = k8s_namespace
        self.canary_names: List[str] = Hitman.get_pods_to_search(canary_names)
        self.canary_ttl: int = int(ttl)
    ...
    def hunt(self) -> Result:
        ...
        return Result(
            ...
            current_commit,
            is_deployable=not deployed_canary_release or self._is_ttl_expired(deployed_at=deployed_canary_release.deployed_at)
        )

    def _build_release(self, pods: List) -> Release:
        # TODO: Move it to hunt method
        repositories: JSON = self.gitlab_client.request_api(path="/projects", params={"search": self.repository_name})
        ...
        for pod in pods:
            ...
            deploy_date: datetime = parse(re.findall(r'Start\sTime:\s+[a-zA-Z]{3},\s([a-zA-Z0-9,\s:+]+)\n', described_pod)[0]).replace(tzinfo=None)
    ...
    def _get_commit(self, repository_id: str, commit_sha: str) -> Optional[Commit]:
        api_commit: JSON = self.gitlab_client.request_api(
        ...
        return Commit(
            ...
            committer=CommitterInfo(
                name=api_commit["committer_name"],
                email=api_commit["committer_email"]
            )
        )
    ...
    def _run_shell_command(self, command: str) -> str:
        return subprocess.check_output(
            ["/bin/bash", "-c", f"kubectl -n {self.k8s_namespace} {command}"]
        ).decode("utf-8")
